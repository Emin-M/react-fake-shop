  ...
  const renderList = card.card.map((item) => {
    return (
      <div className="cover" key={Math.random()}>
        ...
        <div className="count">
          <p className="btnn" onClick={() => decCount(item.id)}>
            -
          </p>
          <span>{item.count}</span>
          <p className="btnn" onClick={() => incCount(item.id)}>
            +
          </p>
        </div>
        <i className="fas fa-trash" onClick={() => del(item.id)}></i>
      </div>
    );
  });
    ...
    <div>
      ...
      <div className={hr ? "total" : "total totall"}>
        <div className="totalBtns">
          ...
          <button onClick={() => clearCard()} className="btn btn-danger">
            Clear Shopping Card
          </button>
        </div>
        <div className="totalPrice">
          ...
          <h4>Shipping Fee : {hr ? 3.24 : 0.0} $</h4>
          <hr className="hrrr" />
          <h1 style={{ fontWeight: "600" }}>
            Order Total: $ {Number(totalPrice.toFixed(2)) + 3.24}
          </h1>
        </div>
        <button className="btn btn-lg btn-success">Make a Payment</button>
      </div>
    </div>
  );
};
